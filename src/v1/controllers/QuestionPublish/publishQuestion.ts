...
const publishQuestion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (questionInfo.status == 'live') {
      ...
      logger.error({ code, apiId, question_id, message: ` Question Published in this id:${question_id}` });
      return res.status(httpStatus.CONFLICT).json(errorResponse(apiId, httpStatus.CONFLICT, `Question Published in this id:${question_id}`, code));
    }

    await publishQuestionById(question_id);

    logger.info({ apiId, question_id, message: 'Question Published successfully' });
    return res.status(httpStatus.OK).json(successResponse(apiId, { data: `Question id:${question_id} published Succesfully` }));
  } catch (error: any) {
