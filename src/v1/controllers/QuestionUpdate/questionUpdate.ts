...
const questionUpdate = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updateQuestionInfo = await updateQuestion(question_id, requestBody.Question);

    // client Response
    if (!updateQuestionInfo.error) {
      return res.status(httpStatus.OK).json(successResponse(apiId, { message: 'Question update successfully', identifier: question_id }));
